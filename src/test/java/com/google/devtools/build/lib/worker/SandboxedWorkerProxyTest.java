...
public class SandboxedWorkerProxyTest {
  ...
  public void prepareExecution_createsFilesInSandbox() throws IOException, InterruptedException {
    ...
    Path workDir = workerBaseDir.getChild(
        "Mnem-multiplex-worker-" + multiplexerId + "-workdir").getChild("execroot"
    );
    Path sandboxDir =
        workDir
            .getChild("__sandbox")
  ...
  public void differentProxiesSameMultiplexerHaveSameWorkDir()
    ...
    Path expectedWorkDirProxyOne = workerBaseDir.getChild(
        "Mnem-multiplex-worker-" + multiplexerIdProxyOne + "-workdir"
    ).getChild("execroot");

    int multiplexerIdProxyTwo = proxyTwo.workerMultiplexer.getMultiplexerId();
    Path expectedWorkDirProxyTwo = workerBaseDir.getChild(
        "Mnem-multiplex-worker-" + multiplexerIdProxyTwo + "-workdir"
    ).getChild("execroot");
  ...
  public void differentProxiesDifferentMultiplexerSameMnemHaveDifferentWorkDirs()
    ...
    ImmutableMap.Builder<String, String> req = WorkerTestUtils.execRequirementsBuilder(sharedMnemonic);
    ...
    Path expectedWorkDirOne = workerBaseDir.getChild(
        sharedMnemonic + "-multiplex-worker-" + multiplexerIdOne + "-workdir"
    ).getChild("execroot");
    ...
    Path expectedWorkDirTwo = workerBaseDir.getChild(
        sharedMnemonic + "-multiplex-worker-" + multiplexerIdTwo + "-workdir"
    ).getChild("execroot");

    System.out.println("Workdir one: " + proxyOneMultiplexerOne.workDir.toString());
    System.out.println("Workdir two: " + proxyOneMultiplexerTwo.workDir.toString());
