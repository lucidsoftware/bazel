...
public class WorkerMultiplexerManager {
   ...
   * the determining the workdir for the multiplexer process. This is analogous to the
   * {@code pidCounter} in {@code WorkerFactory}. It is ok to use an AtomicInteger here as
   * well for the same reasons: it is only incremented when spawning a new multiplexer, so
   * even in the worst case of workers quitting after each action it shouldn't overflow.
   */
  ...
  public static synchronized WorkerMultiplexer getInstance(WorkerKey key, Path logFile) {
    InstanceInfo instanceInfo =
        multiplexerInstance.computeIfAbsent(
            key, k -> new InstanceInfo(new WorkerMultiplexer(logFile, k, multiplexerIdCounter.getAndIncrement())));
